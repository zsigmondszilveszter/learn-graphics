...
void draw_triangle(GM::Triangle tr, GM::Triangle old_tr, uint32_t color) {
    ...
    for (int32_t y=upper_bound; y <= lower_bound; y+=BUFFER_SLICE) {
        ...
        workers[worker_index]->addWorkBlocking({ left_bound, right_bound, y, 
                std::min(y + BUFFER_SLICE, lower_bound), &tr, color, bg_color });
    }
...
int main(int argc, char **argv) {
    ...
    double smaller_screen_dimension = std::min(drmUtil.mdev->width, drmUtil.mdev->height);
    ...
    for ( uint32_t i = 0; i < processor_count; i++) {
        workers.push_back(new Szilv::LineDrawer(i,drmUtil.mdev->width, drmUtil.mdev->height, drmUtil.mdev->map));
    }
